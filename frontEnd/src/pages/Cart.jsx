...
const Cart = () => {
  ...
  const { cart, userProfile, products } = useSelector(
  ...
  const handleRemoveOneFromCart = (pro) => {
    if (pro.qty > 1) {
      ...
      dispatch(
        updateProductById({ productId: pro._id, updatedData: updateCart })
      );
    }
  ...
  const handleMoveToWishlist = (pro) => {
    ...
    const updateCart = { cart: false };
    dispatch(
      updateProductById({ productId: pro._id, updatedData: updateWishList })
    );
    dispatch(
      updateProductById({ productId: pro._id, updatedData: updateCart })
    );
    ...
    setTotalAmount(
      cart.reduce(
        (acc, curr) =>
          (acc +=
            curr.price * curr.qty -
            (curr.price * curr.qty * curr.discount) / 100),
        deliveryCharge
      )
    );
  }, [cart]);
    ...
    <div>
      ...
      <main className="container">
        <h1 className="text-center">
          My Cart Items ({cart.reduce((acc, curr) => (acc += curr.qty), 0)})
        </h1>
        {cart.reduce((acc, curr) => (acc += curr.qty), 0) ? null : (
          <p className="text-center mt-4 text-secondary fs-5">
            Your cart is empty add some items to see cart order details.
          </p>
        )}
        <div>
                      ...
                      <div className="row g-0">
                        ...
                        <div className="col-md-8">
                          <div className="card-body">
                            ...
                            <p className="card-text">
                              <button
                                className="btn btn-sm me-2 border-secondary rounded"
                                onClick={() => handleRemoveOneFromCart(pro)}
                              >
                                -
                              </button>
                              {pro.qty}
                              <button
                                className="btn btn-sm ms-2 border-secondary rounded"
                            ...
                            <div className="d-grid gap-2 col-6 mx-3">
                              <button
                                ...
                                type="button"
                                onClick={() => handleMoveToWishlist(pro)}
                              >
                                Move to Wishlist
                              </button>
                              <button
                                ...
                                type="button"
                                onClick={() => handleRemoveFromCart(pro._id)}
                              >
                                Remove from Cart
                              </button>
                            </div>
                ...
                <div className="card">
                  <div className="card-body">
                    ...
                    <form>
                      ...
                      <input
                        type="text"
                        value={updateAddress.addresslineOne}
                        className="form-control"
                        name="addresslineOne"
                        onChange={(e) => handleChangeOnUpdateAddress(e)}
                      />
                      ...
                      <input
                        className="form-control"
                        value={updateAddress.addresslineTwo}
                        type="text"
                        name="addresslineTwo"
                        onChange={(e) => handleChangeOnUpdateAddress(e)}
                      />
                      ...
                      <div className="d-flex justify-content-between">
                        ...
                        <button
                          type="submit"
                          className="btn btn-sm btn-success"
                          onClick={(e) => handleUpdateAddress(e)}
                        >
                          Update Address
                        </button>
                      </div>
                        ...
                        {userProfile.addresses.map((add, index) => (
                          <div key={add._id || add.id}>
                            ...
                            <button
                              ...
                              onClick={() => handleDeleteAddress(add)}
                            >
                              Delete
                            </button>
                            <br />
                            <label>
                              ...
                              />
                              {add.addresslineOne} {add.addresslineTwo},{" "}
